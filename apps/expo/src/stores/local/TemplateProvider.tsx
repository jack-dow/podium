...
type TemplateExerciseNew = Omit<TemplateExerciseCreate, 'position'>;
type TemplateExerciseUpdated = Omit<TemplateExerciseUpdate, 'position'>;

type TemplateSetNew = Omit<TemplateSetCreate, 'position'>;
type TemplateSetUpdated = Omit<TemplateSetUpdate, 'position'>;
  ...
  DEFAULT_PROPS = produce(DEFAULT_PROPS, (draft) => {
    if (initProps?.templateExercises) {
      for (const templateExercise of initProps.templateExercises) {
        ...
        // ADd the position of the existing template exercise
        draft.templateExercises.positions[id] = position;
      }
    }
      ...
      api: {
        ...
        updateTemplateExercise: (updatedTemplateExercise) =>
          set((template) => {
            ...
            // Is existing
            if (template.templateExercises.existing.includes(id)) {
              template.templateExercises.existing = template.templateExercises.existing.filter(
        ...
        removeTemplateExercise: (templateExerciseId) => {
          set((template) => {
            // Is Existing
            if (template.templateExercises.existing.includes(templateExerciseId)) {
              template.templateExercises.existing = template.templateExercises.existing.filter(
        ...
        updateTemplateSet: (updatedTemplateSet) =>
          set((template) => {
            ...
            // Is Existing
            if (template.templateSets.existing.includes(id)) {
              template.templateSets.existing = template.templateSets.existing.filter((existingId) => existingId !== id);
        ...
        removeTemplateSet: (templateSetId) =>
          set((template) => {
            // Is existing
            if (template.templateSets.existing.includes(templateSetId)) {
              template.templateSets.existing = template.templateSets.existing.filter((id) => id !== templateSetId);
        ...
        getSubmittableTemplate: () => {
          ...
          const templateUpdate: TemplateUpdate = {
            ...
            templateExercises: {
              ...
              updated: Object.values(template.templateExercises.updated),
...
export function TemplateProvider({ children, id }: React.PropsWithChildren<TemplateProviderProps>) {
  ...
  if (queriesEnabled && isTemplateLoading) {
    return (
      <SafeAreaView className="items-center justify-center space-y-md">
        <Loader />
  ...
  if (isTemplateError || (queriesEnabled && !template)) {
    return (
...
export function useTemplateExercise(templateExerciseId: TemplateExerciseId) {
  const isExistingTemplateExercise = useTemplateContext((s) =>
    s.templateExercises.existing.includes(templateExerciseId),
  );
  ...
  const {
    ...
    { id: templateExerciseId },
    { enabled: !newTemplateExercise && (isExistingTemplateExercise || !!updatedTemplateExercise) },
  );
  ...
  } = trpc.exercise.byId.useQuery({ id: newTemplateExercise!.exerciseId }, { enabled: !!newTemplateExercise });

  if (isExistingTemplateExercise) {
    return {
      templateExercise,
      isLoading: isTemplateExerciseLoading,
      isError: isTemplateExerciseError,
  ...
  // if its an existing template exercise but has been updated
  ...
  // If it's a new template exercise
...
export function useTemplateSet(templateSetId: TemplateSetId) {
  const isExistingTemplateSet = useTemplateContext((s) => s.templateSets.existing.includes(templateSetId));
  ...
  } = trpc.templateSet.byId.useQuery(
    { id: templateSetId },
    { enabled: !newTemplateSet && (isExistingTemplateSet || !!updatedTemplateSet) },
  );

  // Only existing template set
  if (isExistingTemplateSet) {
    return {
      templateSet,
      isLoading: isTemplateSetLoading,
      isError: isTemplateSetError,
  ...
  // Template set exists but has been updated
